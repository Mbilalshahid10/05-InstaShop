...
const HomePage = ()=>{
  ...
  useEffect( () => {
    if (role === "Client"){
      axios.get('https://colleeasy.herokuapp.com/allInfProfiles', {withCredentials: true})
  ...
  useEffect( () => {
    if (role === "Client"){
      axios.post('https://colleeasy.herokuapp.com/RecommendationstoClient', client_email ,{withCredentials: true})
      .then(response => response.data)
      .then(data => {
        console.log(data)
        const temp = data.slice(0,5)
