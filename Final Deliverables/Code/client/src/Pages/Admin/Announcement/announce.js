...
const Announce = ()=>{
  ...
  const navigate = useNavigate()
    ...
    const announcement = async (e) =>{
      ...
      await axios.post('https://colleeasy.herokuapp.com/create_announcement', announcements, {withCredentials: true}).then(navigate('/home')).catch(err=>console.log(err))
  }

  const viewannouncement = async (e) =>{
    e.preventDefault()
    navigate('/viewannouncement')
    // await axios.get('https://colleeasy.herokuapp.com/get_announcement',{withCredentials: true})
